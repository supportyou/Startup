...
!(function initHeader($){
    ...
    if(!!navigator.userAgent.match(/36kr/)){
        ...
        $('link[href*=common-header]').remove();
        return;
    }
...
//
    ...
    var Services = {
        ...
        login: function(){
            ...
            setTimeout(function(){
                ...
                // 是否在passport页面
                var hostname = location.hostname.indexOf('passport.36kr.com')>-1;

                if(!hostname){
                    location.href = 'https://passport.36kr.com/pages/?ok_url='+backUrl+'#/login?pos=header';
                    return;
                }
                // 如果在登录页,则结束
                ...
                if(location.href.indexOf('#/login') === -1) {
                    location.href = "https://passport.36kr.com/pages/" + location.search + "#/login?pos=header"
                }
        ...
        // 检查cookie
        checkoutInvestorValidateNum : function(){
            var num = '';
            document.cookie.replace(/kwlo_iv=(\d)h?/i, function(c1, c2){
                num = c2;
            });

            if(!num){
                document.cookie='kwlo_iv=1h; domain=.36kr.com; path=/;';
            }else if((num-0) <= 3){
                num = (num - 0 + 1) + 'h';
                document.cookie='kwlo_iv='+num+'; domain=.36kr.com; path=/;';
            }
        },
        signup:function(){
            var backUrl = encodeURIComponent(location.href);
            ...
            if(window._hmt){
                _hmt.push(['_trackPageview', "/register"]);
            }
            if(window.krtracker){
                krtracker('trackPageView', '/register');
            }

            setTimeout(function(){
                // https://z.36kr.com/investorValidate
                // 是否在passport页面
                var hostname = location.hostname.indexOf('passport.36kr.com')>-1;
                if(!hostname) {
                    location.href = 'https://passport.36kr.com/pages/?ok_url='+backUrl+'#/register?pos=header';
                    return;
                }
                // 如果在注册页,则结束
                ...
                if(location.href.indexOf('#/register') === -1) {
                    location.href = "https://passport.36kr.com/pages/" + location.search + "#/register?pos=header"
                }
            },100);
        },
        logout: function(){
    ...
    */
    ...
    //模板
    var tpls = {
        menu: ('<ul class="sub-nav">    <li class="search-item">        <div class="search-form-wrapper" href="javascript:void(0)" data-stat-hover="sousuo">            <div class="search-form-inner">                <i class="headericon-header-search"></i>                <div class="search-form-box">                    {@if chl!=\'news\'&&chl!=\'insight\'}                    <form action="" class="J_searchForm">                        <input type="text" placeholder="搜索" ng-model="keyword">                    </form>                    {@/if}                    {@if chl==\'insight\'}                    <form action="" class="J_searchForm">                        <input type="text" placeholder="请输入公司名称" ng-model="keyword">                    </form>                    {@/if}                    {@if chl==\'news\'}                    <form action="" class="J_searchForm">                        <input type="text" placeholder="搜索" ng-model="keyword">                    </form>                    {@/if}                </div>                <button class="do-submit" data-stat-click="sousuo">搜索</button>                <button class="do-submit do-submit-icon headericon-header-search" data-stat-click="sousuo"></button>                <button class="headericon-close close-icon" type="button" data-stat-click="guanbisousuo"></button>            </div>            <span></span>        </div>        <!--<div class="search-wrap pop-up">-->            <!--<div class="searchbar">-->                <!--{@if chl!=\'news\'&&chl!=\'insight\'}-->                <!--<form action="" class="J_searchForm">-->                    <!--<input type="text" placeholder="创业公司、投资人、投资机构" ng-model="keyword">-->                    <!--<button class="headericon-header-search search-icon" type="submit" data-stat-click="sousuo"></button>-->                    <!--<button class="headericon-close close-icon" type="button" data-stat-click="guanbisousuo"></button>-->                <!--</form>-->                <!--{@/if}-->                <!--{@if chl==\'insight\'}-->                <!--<form action="" class="J_searchForm">-->                    <!--<input type="text" placeholder="请输入公司名称" ng-model="keyword">-->                    <!--<button class="headericon-header-search search-icon" type="submit" data-stat-click="sousuo"></button>-->                    <!--<button class="headericon-close close-icon" type="button" data-stat-click="guanbisousuo"></button>-->                <!--</form>-->                <!--{@/if}-->                <!--{@if chl==\'news\'}-->                <!--<form action="" class="J_searchForm">-->                    <!--<input type="text" placeholder="搜索" ng-model="keyword">-->                    <!--<button class="headericon-header-search search-icon" type="submit" data-stat-click="sousuo"></button>-->                    <!--<button class="headericon-close close-icon" type="button" data-stat-click="guanbisousuo"></button>-->                <!--</form>-->                <!--{@/if}-->            <!--</div>-->        <!--</div>-->    </li>    <!--<li class="app-download">-->        <!--<a href="http://download.36kr.com" data-stat-hover="kehuduan"><i class="headericon-header-app"></i> 客户端</a>-->        <!--<div class="mobile-wrap pop-up">-->            <!--<img class="qr" src="//sta.36krcnd.com/common-module/common-header/images/appqr.png" alt=""/>-->            <!--<div class="btns">-->                <!--<span>下载36氪客户端</span>-->                <!--<a href="https://appsto.re/cn/2LNxJ.i" data-stat-click="ios"><i class="headericon-apple"></i> App Store</a>-->                <!--<a href="http://fir.im/45sa" data-stat-click="android"><i class="headericon-android"></i> Android</a>-->            <!--</div>-->        <!--</div>-->    <!--</li>-->    {@if !isLogin}    <li class="login-actions">        <div class="login" >            <i class="headericon-header-user"></i>            <a href="javascript:void(0)" class="J_login login-link" data-stat-click="denglu">登录</a>            <b> / </b>            <a href="javascript:void(0)" class="J_signup login-link" data-stat-click="zhuce">注册</a>        </div>    </li>    {@else}    <li class="msg-menu">        <a href="${ucHost}/#/uc/message/inbox/followed/" data-stat-click="xiaoxi" data-stat-hover="xiaoxi"><i class="headericon-header-msg J_msgIcon"></i> 消息</a>        <div class="msg-wrap pop-up J_msgCountWrap"></div>    </li>    <li class="user-menu">        <a href="${ucHost}/#/uc/account/basic" class="head-avatar" data-stat-click="touxiang" data-stat-hover="touxiang"><img src="${user.avatar_url}" alt="${user.name}"/></a>        <div class="menu-wrap pop-up">            <a class="brief" href="${ucHost}/#/uc/account/basic" data-stat-click="zhanghushezhi">                <img src="${user.avatar_url}" alt="${user.name}"/>                <span>${user.name}</span>            </a>            <div class="menu">                {# @if coInvestor}                <a href="${ucHost}/#/uc/coinvest" data-stat-click="wodedingdan">我的订单</a>                {# @/if}                {@if krPlusId}                <a href="${rongHost}/userinfo/${krPlusId}/edit" data-stat-click="wodezhuye">我的主页</a>                {@/if}                <a href="${ucHost}/#/uc/favorite " data-stat-click="wodeshoucang">我的收藏</a>                {@if krPlusId}                <a href="${rongHost}/company/create" data-stat-click="chuangjiangongshi">创建公司</a>                <a href="${ucHost}/#/uc/mycompany" data-stat-click="woguanlidegongsi">我管理的公司</a>                <!--<a href="${ucHost}/#/uc/follow/1" data-stat-click="woguanzhudegongsi">我关注的公司</a>-->                {@/if}                {@if role&&role!=\'reader\'}                    {@if role!=\'operator\'}                    <a href="${krWriterHost}/posts/new" data-stat-click="zhuanxiewenzhang" target="_blank">撰写文章</a>                    {@/if}                    {@if role!=\'investor\'&&role!=\'organization\'&&role!=\'entrepreneur\'}                        {@if role!=\'operator\'}                            <a href="${krHost}/krypton/posts/myown" data-stat-click="wodewenzhang">我的文章</a>                            <a href="${krHost}/krypton/posts/draft" data-stat-click="wodecaogao">我的草稿</a>                        {@/if}                        <a href="${krHost}/krypton/dashboard" data-stat-click="jinruhoutai">进入后台</a>                    {@else}                        <a href="${ucHost}/#/uc/article-manager" data-stat-click="wodewenzhang">我的文章</a>                    {@/if}                {@/if}                                {@if leadInvestor}                <a href="${ucHost}/#/uc/lead" data-stat-click="wolingtoudegongsi">我领投的公司</a>                {@/if}                <!--<a href="${krHost}/pages/contribute" data-stat-click="tougao">向36氪投稿</a>-->                {@if !investor}                <!--<a href="${rongHost}/setting/vip" data-stat-click="renzhenggentouren">认证投资人</a>-->                {@/if}                {@if !coInvestor}                <a href="${zhongHost}/investorValidate" data-stat-click="renzhengtouziren">认证跟投人</a>                {@/if}                <a href="${ucHost}/#/uc/account/basic" data-stat-click="zhanghaoshezhi">账号设置</a>                <a href="javascript:void(0)" class="J_logout" data-stat-click="tuichu">退出</a>            </div>        </div>    </li>    {@/if}</ul>'),
    ...
    function renderRightNav(data){
        ...
        data.avatar_url = data.avatar_url ? data.avatar_url.replace(/!\d+/i, '!100') : '';
        ...
        // 注册按钮
        $('.J_signup', wrapper).bind('click touchend', function(e){
            e.preventDefault();
            Services.signup();
        });
    ...
    function getMsgCount(lastResult){
        ...
        Services.getMsgCount(function(data){
            ...
            var durationTime = (120 + Math.random()*120)*1000;
            setTimeout(function(){
     ...
     */
    function initSearch(){
        var form = $('.J_commonHeaderWrapper .search-form-box');
        var input = form.find('input');
        var wrap = form.parents('li');
        var otherWrap = wrap.siblings();
        var submit_btn = $('.do-submit');
        ...
        form.submit(function(e){
            ...
            $('.J_commonHeaderWrapper .close-icon').trigger('touchend');
            krtracker('trackEvent', 'keyup', "header.search");
            setTimeout(function(){
                var href = '';

                if(activeName=="news" || activeName=="newsflashes" || typeof activeName == 'undefined'){
                    href = krHost + '/search/articles/' + encodeURIComponent(keyword);
                }else if(activeName=="insight"){
                    href = insightHost+'/#/result?keyword='+encodeURIComponent(keyword);
                }else{
                    href = krHost + '/search/companies/' + encodeURIComponent(keyword);
                }
                location.href = href;
            },300);
        });
    ...
    function triggerBind(){
        ...
        $('.J_commonHeaderWrapper .close-icon').bind('mouseup touchend', function(e){
            e.preventDefault();
            setTimeout(function(){
                var wrapper = $('.J_commonHeaderWrapper');
